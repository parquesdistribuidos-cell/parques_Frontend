...
export default function LobbyPage() {
  ...
  const [vista, setVista] = useState<"menu" | "crear" | "unirse" | "sala">(
    "menu",
  );
  ...
  const crearSala = () => {
    wsClient.send("CREAR_SALA", {
      nombre: nombreInput || `Sala de ${username}`,
    });
    setNombreInput("");
  };
  ...
  const marcarListo = () => {
    if (!miColor) {
      setError("Elige un color primero");
      return;
    }
      ...
      <div className="max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              >
                ...
                <div className="text-gray-400 text-sm">
                  Crea una sala nueva e invita a otros jugadores
                </div>
              </button>
              <button
                onClick={() => {
              ...
              >
                ...
                <div className="text-gray-400 text-sm">
                  Entra a una sala existente con su PIN
                </div>
              </button>
            </div>
          </div>
        ...
        {vista === "crear" && (
          <div className="bg-white/10 rounded-2xl p-8 border border-white/20 max-w-md mx-auto">
            ...
            <div className="flex gap-3">
              <button
                onClick={crearSala}
                className="flex-1 py-3 bg-purple-600 hover:bg-purple-500 text-white font-semibold rounded-xl"
              >
                Crear
              </button>
              <button
        ...
        {vista === "unirse" && (
          <div className="space-y-4">
            ...
            {(
              salas as Array<{ pin: string; nombre: string; jugadores: number }>
            ).length > 0 && (
              ...
              <div className="bg-white/10 rounded-2xl p-6 border border-white/20">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    ...
                    <div className="text-gray-400 text-sm mt-1">
                      PIN:{" "}
                      <span className="font-mono text-purple-400 text-lg tracking-widest">
                        {pin}
                      </span>
                    </div>
                  </div>
                ...
                <div className="space-y-2">
                  {jugadores.map((j) => (
                    ...
                    >
                      <div
                        className={`w-4 h-4 rounded-full ${j.color ? `ficha-${j.color}` : "bg-gray-600"}`}
                      />
                      <span className="text-white flex-1">{j.username}</span>
                      <span className="text-sm text-gray-400">
                        {j.color || "sin color"}
                      </span>
                      <span
                        className={`text-xs px-2 py-1 rounded-full ${j.listo ? "bg-green-500/20 text-green-400" : "bg-gray-500/20 text-gray-400"}`}
                      >
                ...
                <div className="grid grid-cols-2 gap-3">
                  {COLORES.map((c) => {
                    ...
                    return (
                      <button
                        key={c}
                        onClick={() => disponible && elegirColor(c)}
                        disabled={!disponible}
                        className={`p-4 rounded-xl border-2 transition-all flex items-center gap-3 ${
                          seleccionado
                            ? `${COLOR_STYLES[c]} scale-105 shadow-lg`
                            : disponible
