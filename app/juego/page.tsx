...
export default function JuegoPage() {
  ...
  const {
    token, usuarioId, username, jugadores, pin, miColor,
    tablero, turnoActual, dadoA, dadoB, esPar,
    ...
    const unsub = wsClient.onMessage((msg) => {
      ...
      switch (msg.type) {
        ...
        case "DADOS_RESULTADO":
          ...
          if (p.fase !== "tirada_inicial") {
            setDados(p.dado_a as number, p.dado_b as number, p.es_par as boolean);
            addLog(`🎲 ${p.username} sacó ${p.dado_a} - ${p.dado_b}${p.es_par ? " ¡PAR!" : ""}`);
          } else {
        ...
        case "FICHA_MOVIDA":
          ...
          if (msg.type === "FICHA_MOVIDA") {
            addLog(`♟️ ${p.username} movió ficha ${p.ficha_id}`);
          }
          setMovimientos([]);
          setAccion(null);
          break;
        ...
        case "FICHA_CAPTURADA":
          addLog(`💥 Ficha ${p.ficha_capturada_id} capturada por ${p.captor_username}!`);
        ...
        case "TRES_PARES":
          addLog(`🔥 ${p.username} sacó 3 pares seguidos! Debe quemar una ficha.`);
          setNotificacion(p.mensaje as string);
          break;

        case "FICHA_QUEMADA":
          addLog(`🔥 Ficha ${p.ficha_id} eliminada por 3 pares consecutivos`);
          break;
        ...
        case "PARTIDA_TERMINADA":
          setGanador({ username: p.ganador_username as string, color: p.ganador_color as string });
  ...
  function moverFicha(fichaId: number, valor?: number) {
    if (!esMiTurno) return;
    const movs = (movimientosLegales as Array<{ ficha_id: number; valor: number; dado: string }>)
      .filter(m => m.ficha_id === fichaId);
    if (!movs.length) return;
    const mov = valor ? movs.find(m => m.valor === valor) || movs[0] : movs[0];
    ...
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <div className="bg-white/10 backdrop-blur rounded-3xl p-12 text-center border border-white/20 max-w-md">
          ...
          <h1 className="text-3xl font-bold text-yellow-400 mb-2">¡{ganador.username} ganó!</h1>
          <p className="text-gray-400 mb-6">Color: {ganador.color} {COLOR_EMOJI[ganador.color]}</p>
          <div className="flex gap-4 justify-center">
            <button onClick={() => { resetTodo(); router.push("/lobby"); }}
              className="px-6 py-3 bg-purple-600 hover:bg-purple-500 text-white font-semibold rounded-xl">
              Nueva partida
            </button>
            <button onClick={() => { resetTodo(); router.push("/perfil"); }}
    ...
    <div className="min-h-screen flex flex-col">
        ...
        <div className="flex items-center gap-4">
          {jugadores.map(j => (
            <div key={j.id} className={`flex items-center gap-2 px-3 py-1 rounded-full text-sm transition-all ${
              ...
              <span className="text-white">{j.username} {j.es_bot && "🤖"}</span>
            </div>
          ))}
        </div>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden min-h-0">
        ...
        <div className="w-72 bg-black/20 border-r border-white/10 p-4 space-y-4 overflow-y-auto">
          {/* Turno actual */}
          <div className={`rounded-xl p-4 border ${esMiTurno ? "bg-green-500/20 border-green-500/50" : "bg-white/5 border-white/10"}`}>
            <div className="text-xs text-gray-400 mb-1">Turno actual</div>
            <div className="text-white font-semibold">
              {jugadorActual ? `${COLOR_EMOJI[jugadorActual.color]} ${jugadorActual.username}` : "..."}
            </div>
            {esMiTurno && <div className="text-green-400 text-sm mt-1">¡Es tu turno!</div>}
          </div>

          {/* Dados */}
          <div className="bg-white/5 rounded-xl p-4 border border-white/10 text-center">
            <div className="text-gray-400 text-xs mb-3">Dados</div>
            <div className={`flex justify-center gap-4 text-5xl mb-3 transition-all ${dadosAnimando ? "scale-125" : ""}`}>
          ...
          {esMiTurno && (movimientosLegales as Array<{ ficha_id: number; valor: number; dado: string; destino_descripcion: string }>).length > 0 && (
            <div className="bg-white/5 rounded-xl p-4 border border-white/10">
              <div className="text-gray-400 text-xs mb-3">Movimientos posibles</div>
              <div className="space-y-2">
                {(movimientosLegales as Array<{ ficha_id: number; valor: number; dado: string; destino_descripcion: string }>)
                  .slice(0, 6).map((m, i) => (
                  <button key={i} onClick={() => moverFicha(m.ficha_id, m.valor)}
                    ...
                    Ficha {m.ficha_id % 10} → {m.destino_descripcion.substring(0, 25)}
                  </button>
                ))}
              </div>
            </div>
          ...
          {esMiTurno && (
            <div className="space-y-2">
              <button onClick={pedirRecomendacion}
              ...
              {recomendacion && (
                <div className="bg-blue-900/40 border border-blue-500/30 rounded-xl p-3">
                  <div className="text-blue-300 text-xs font-semibold mb-1">💡 IA recomienda:</div>
                  {recomendacion.ficha_id && (
                    <div className="text-white text-sm mb-1">
                      Ficha {recomendacion.ficha_id % 10} con dado {recomendacion.dado?.toUpperCase()} ({recomendacion.valor})
                    </div>
                  )}
                  <div className="text-gray-400 text-xs">{recomendacion.justificacion}</div>
                </div>
              )}
            </div>
          )}
        ...
        <div className="flex-1 flex items-center justify-center p-4 bg-gradient-to-br from-purple-900/20 to-blue-900/20">
          <Tablero
            ...
            usuarioId={usuarioId}
            movimientosLegales={movimientosLegales as Array<{ ficha_id: number; valor: number; dado: string; destino_descripcion: string }>}
          ...
          <div ref={chatRef} onScroll={handleChatScroll} className="flex-1 min-h-0 overflow-y-auto p-4 space-y-2">
            ...
            {mensajes.map((m, i) => (
              <div key={i} className={`rounded-xl p-2 text-sm ${
                m.usuario_id === usuarioId ? "bg-purple-600/30 ml-4" : "bg-white/5 mr-4"
              }`}>
